...
function firstLoad() {
  ...
  xhttp.onreadystatechange = function() {
    ...
    if (this.readyState == 4 && this.status == 200) {
      ...
      const stats = playerArray.reduce((stats, player) => {
        ...
        red_cards: {max: 0, bestPlayer: null},
        yellow_cards: {max: 0, bestPlayer: null},
        saves: {max: 0, bestPlayer: null},
        selected_by_percent: {max: 0, bestPlayer: null},
        form: {max: 0, bestPlayer: null},
        transfers_out_event: {max: 0, bestPlayer: null},
        transfers_in_event: {max: 0, bestPlayer: null},
  ...
  xhttp.open("GET", "https://fantasy.premierleague.com/drf/bootstrap-static", true);
  xhttp.send();
}

firstLoad();
//   if (allPlayers !== null && typeof allPlayers === 'object') {
...
function loadPlayers() {
  ...
  xhttp.onreadystatechange = function() {
    ...
    if (this.readyState == 4 && this.status == 200) {
      ...
      output += "<table id='playerList'><thead><tr><th role='columnheader'>Name</th><th role='columnheader'>Team</th><th role='columnheader'>Position</th><th role='columnheader'>Selected by</th><th role='columnheader'>Form</th><th role='columnheader'>Goals</th><th role='columnheader'>Assists</th><th role='columnheader'>Yellow</th><th role='columnheader'>Red</th></tr></thead><tbody>";
...
function indepthPlayer(playerID, playerName) {
  ...
  xhttp.open("GET", "https://fantasy.premierleague.com/drf/element-summary/" + playerID, true);
  xhttp.send();
...
function teamSearch(playerID, playerName, playerTeam) {
 ...
 window.scrollTo(0,0);
      ...
      output += "<h1 class='teamtitle'>" + teamNames[playerTeam] + "</h1>"
      output += "<div class='wrapper'>";
...
// var start = document.getElementById('playername');
// start.focus();
